...
import com.zero.ioshop.userservice.dto.ResponseDto;
...
public class UserServiceImpl implements UserService {
    ...
    public ResponseModel listAll() {
        ...
        return new ResponseModel(MessageStatus.SUCCESS, collect);
    }
    ...
    public ResponseModel findById(Long id) {
        ...
        if (user.isPresent()) {
            return new ResponseModel(MessageStatus.SUCCESS, mapper.map(user, ResponseDto.class));
        }
    ...
    public ResponseModel save(User user) {
        ...
        if (user.getId() == null) {
            ...
            if (save.getId() != null) {
                return new ResponseModel(MessageStatus.SUCCESS, mapper.map(save, ResponseDto.class));
            }
            return new ResponseModel(MessageStatus.COULD_NOT_SAVE_RECORD);
        } else {
            Optional<User> byId = userRepository.findById(user.getId());
            if (byId.isPresent()) {
                return new ResponseModel(MessageStatus.SUCCESS, mapper.map(userRepository.save(user), ResponseDto.class));
            }
    ...
    public ResponseModel deactivate(Long id) {
        ...
        return new ResponseModel(MessageStatus.COULD_NOT_UPDATE_RECORD);
    }
